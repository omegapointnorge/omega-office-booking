...
const OverviewSeatInfo = observer(
  ({ onClose, selectedSeatId }: OverviewSeatInfoProps) => {
    ...
    //TODO: claim type
    const claimId = user?.claims?.find(
      (claim: any) =>
        claim.key ===
    ...
    const executeRecaptcha: () => Promise<string> = async () => {
    ...
    const handleBook = async () => {
      ...
      await bookingStore.createBookingRequest(selectedSeatId, reCAPTCHAtoken);
      onClose();
    };
    ...
    const handleDelete = async () => {
      ...
      if (selectedBooking?.id) {
        await bookingStore.deleteBooking(selectedBooking.id);
        onClose();
      }
    };
    //TODO: ta ut
    const getButtonGroup = () => {
      ...
      if (selectedBooking.id && claimId !== selectedBooking.userId) {
        return (
          <button
            onClick={onClose}
        ...
        return (
          <div className="flex justify-between">
            ...
            {claimId === selectedBooking.userId && (
              <button
