...
public static class WebApplicationSeatExtensions
{
    ...
    {
        app.MapGet("/Seats", (SeatRepository repo) => repo.GetAll())
            .Produces<SeatDto[]>(StatusCodes.Status200OK);

        app.MapGet("/Seat/{SeatId:int}", async (int SeatId, SeatRepository repo) => 
        {
        ...
        app.MapPost("/Seats", async ([FromBody] SeatDetailDto dto, SeatRepository repo) => 
        {
        ...
        app.MapPut("/Seats", async ([FromBody] SeatDetailDto dto, SeatRepository repo) => 
        {       
            if (!MiniValidator.TryValidate(dto, out var errors))
                return Results.ValidationProblem(errors);
        ...
        app.MapDelete("/Seats/{SeatId:int}", async (int SeatId, SeatRepository repo) => 
        {
