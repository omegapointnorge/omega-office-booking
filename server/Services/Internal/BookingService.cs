...
public class BookingService : IBookingService
{
    ...
    public async Task BatchDeleteBookings(int[] bookingIds, UserClaims userClaims)
    {
        try
        {
            var bookingsToDelete = await _bookingRepository.GetAllAsync(booking => bookingIds.Contains(booking.Id));

            foreach (var booking in bookingsToDelete)
            {
                var validationErrors = ValidateUserDeleteBookingRequest(booking, userClaims);
